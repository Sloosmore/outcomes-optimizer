 ...
 */
async function patchProcess(fields) {
    ...
    try {
        ...
        const nativeFetch = globalThis.__nativeFetch ?? fetch;
        ...
        console.warn(`[loop] patchProcess failed: ${err.message}`);
    }
}
/**
 * Update the progress field in the DB for the current process (best-effort, never throws).
 */
export async function updateProgress(workspaceDir) {
 ...
 */
function isCompleteSignal(filePath) {
...
async function checkWaitingStatus() {
    ...
    try {
        ...
        const output = execFileSync(bin, [...prefixArgs, 'process', 'status', '--id', processId, '--json'], { encoding: 'utf-8', timeout: 15000, stdio: ['pipe', 'pipe', 'pipe'] });
        ...
        console.warn(`Warning: could not check process waiting status: ${err.message}`);
        return false;
    }
}
/**
...
function enrichProcessEnv() {
    ...
    try {
        ...
        console.warn(`Warning: could not enrich process env: ${err.message}`);
    }
}
/**
 * Execute the ralph loop - an iterative optimization process using CLI adapter.
 */
export async function executeLoop(config) {
    ...
    if (processId && UUID_RE.test(processId)) {
        try {
            ...
            console.warn(`[loop] process active failed: ${err.message}`);
        }
    }
    for (let i = startEpoch; i <= endEpoch; i++) {
        ...
        const authError = rawOut.includes('Not logged in') || rawOut.includes('Please run /login') || rawOut.includes('Invalid API key') || rawOut.includes('Fix external API key');
        ...
        if (isCompleteSignal(progressFile)) {
            console.log('COMPLETE signal found in progress.md');
            if (enableGit)
                commitEpoch(i, workspace, epochMeta);
            ...
            if (enablePR && process.env.EVAL_BRANCH) {
                try {
                    ...
                    console.error(`openPRAdapter failed: ${err.message}`);
                }
            }
            return { completed: true, epochs: i, sessionIds };
        }
        ...
        if (checkAllStoriesPassed(workspace)) {
            ...
            await markProcessCompleted();
            if (enablePR && process.env.EVAL_BRANCH) {
                try { openPRAdapter({ branch: process.env.EVAL_BRANCH, worktreePath }); } catch (err) { console.error(`openPRAdapter failed: ${err.message}`); }
            }
 ...
 * then upload this epoch's results to the database via the CLI (SKILL_NETWORKS_CLI) process record.
 ...
 */
function commitEpoch(epoch, workspace, meta) {
    ...
    const sh = (cmd) => {
        try {
            ...
            console.warn(`[commitEpoch] sh failed: ${cmd}`, err.message);
            return false;
        }
    };
    ...
    try {
        ...
        if (existsSync('./results'))
            sh(`find results -name ".git" -type d -exec rm -rf {} + 2>/dev/null`);
        // Stage and commit if there are changes
        sh(`git add -f workspace/`);
        if (existsSync('./results'))
            sh(`git add -f results/`);
        if (!sh('git diff --cached --quiet')) {
            execSync(`git commit -m "Epoch ${epoch}"`, { stdio: 'inherit' });
        ...
        if (sh(`git push --force-with-lease origin "${branch}" 2>/dev/null || git push origin "${branch}"`)) {
        ...
        if (process.env.EVAL_CAMPAIGN_ID)
            console.warn('⚠️  EVAL_CAMPAIGN_ID is deprecated — use EVAL_PROCESS_ID');
        if (process.env.EVAL_CAMPAIGN)
            console.warn('⚠️  EVAL_CAMPAIGN is deprecated — use EVAL_PROCESS');
        const processId = process.env.EVAL_PROCESS_ID || process.env.EVAL_CAMPAIGN_ID;
        ...
        if (processId || processName) {
            ...
            try {
                ...
                console.warn(`⚠️ duoidal process record failed for epoch ${epoch} — results remain in git:`, err.message);
            }
        }
    }
    catch (e) {
        console.error(`⚠️ commitEpoch failed: ${e.message}`);
    }
    finally {
        sh(`rm -rf "${saveDir}"`);
    }
 ...
 */
export function checkAllStoriesPassed(workspace) {
    try {
        const stateFile = resolve(workspace, 'state.json');
        if (!existsSync(stateFile))
            return false;
        const state = JSON.parse(readFileSync(stateFile, 'utf-8'));
        const activePrd = state?.active_prd;
        if (!activePrd || typeof activePrd !== 'string')
            return false;
        if (!/^[\w-]+$/.test(activePrd))
            return false;
        ...
        if (!Array.isArray(stories) || stories.length === 0)
            return false;
        return stories.every((s) => typeof s === 'object' && s !== null && s.passes === true);
    }
    catch (err) {
        console.warn(`[loop] checkAllStoriesPassed: failed to evaluate — ${err.message}`);
        return false;
    }
}
