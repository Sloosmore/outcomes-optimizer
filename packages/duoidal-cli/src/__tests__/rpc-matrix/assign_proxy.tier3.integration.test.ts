...
describe('tier3: assign_proxy', () => {
  it('executes RPC and returns success', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      // identity→proxy is the valid 'proxy' link pairing
      ...
      assert(identityResult.ok, `create_identity failed: ${JSON.stringify(identityResult.data)}`)
      ...
      assert(proxyResult.ok, `create_proxy failed: ${JSON.stringify(proxyResult.data)}`)
