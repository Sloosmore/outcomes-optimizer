...
describe('tier2: fetch_skill_content', () => {
  it('fetches skill content via skills get path and returns content', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const skillResult = await callRpcService('create_skill', { p_name: `skill-${suffix}`, p_config: { content: '# Test Skill Content' }, p_project_id: projectId })
      ...
      const result = await callRpcService('fetch_skill_content', { p_skill_resource_id: skillId! })
