...
describe('tier3: create_skill', () => {
  it('executes RPC and returns skill resource ID', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const result = await callRpcService('create_skill', { p_name: `skill-${suffix}`, p_config: { content: '# Test Skill' }, p_project_id: projectId })
