...
describe('tier3: delete_resource', () => {
  it('executes RPC and returns success', async () => {
    await withTestUser(async ({ jwt, assert }) => {
      const suffix = Math.random().toString(36).slice(2, 10)

      // Create a project to delete
      const projResult = await callRpcService('create_project', { p_name: `proj-del-${suffix}` })
      ...
      const result = await callRpc('delete_resource', { p_resource_id: projectId! }, jwt)
