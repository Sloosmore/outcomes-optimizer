/**
 * Read-path: show a resource by name.
 *
 * Queries the resources table via Supabase REST API by name.
 */
...
describe('read-path: show by name', () => {
  it('retrieves a resource by its exact name', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      assert(resource!['name'] === projectName, `Expected name=${projectName}, got: ${JSON.stringify(resource)}`)
