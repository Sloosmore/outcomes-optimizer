/**
 * Read-path: fetch a resource by UUID.
 *
 * Queries the resources table via Supabase REST API by ID.
 */
...
async function getResourceById(id: string): Promise<Record<string, unknown> | null> {
  const url = `${SUPABASE_URL}/rest/v1/resources?id=eq.${id}&limit=1`
...
describe('read-path: resource get by ID', () => {
  it('fetches a resource by its UUID and returns correct fields', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const resource = await getResourceById(projectId!)
      assert(resource !== null, `Expected to find resource with id=${projectId}`)
      ...
      assert(resource!['status'] !== undefined, 'Expected status field to be present')
