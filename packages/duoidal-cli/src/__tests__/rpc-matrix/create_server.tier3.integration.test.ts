...
describe('tier3: create_server', () => {
  it('executes RPC and returns server resource ID', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const result = await callRpcService('create_server', { p_name: `server-${suffix}`, p_config: {}, p_project_id: projectId })
