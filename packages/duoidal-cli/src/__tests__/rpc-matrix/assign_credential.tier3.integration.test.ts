...
describe('tier3: assign_credential', () => {
  it('executes RPC and returns success', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      // server→credential is a valid 'credential' link pairing
      const serverResult = await callRpcService('create_server', { p_name: `server-${suffix}`, p_config: { ip: '1.2.3.4' }, p_project_id: projectId })
      assert(serverResult.ok, `create_server failed: ${JSON.stringify(serverResult.data)}`)
      ...
      assert(credResult.ok, `create_credential failed: ${JSON.stringify(credResult.data)}`)
