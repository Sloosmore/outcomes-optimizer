...
describe('tier3: update_skill_content', () => {
  it('executes RPC and returns resource ID', async () => {
    await withTestUser(async ({ jwt, assert }) => {
      ...
      const skillResult = await callRpcService('create_skill', { p_name: `skill-${suffix}`, p_config: { content: '# Initial Content' }, p_project_id: projectId })
      assert(skillResult.ok, `create_skill failed`)
      const resourceId = (Array.isArray(skillResult.data) ? skillResult.data[0] : skillResult.data as Record<string, unknown>)?.['skill_resource_id'] as string | undefined
      assert(typeof resourceId === 'string', `Expected skill ID`)

      const result = await callRpc('update_skill_content', {
        p_resource_id: resourceId!,
        p_content: '# Updated Skill\n\nThis is updated content for the tier3 integration test suite. It must be longer than one hundred characters to pass validation.',
      }, jwt)
