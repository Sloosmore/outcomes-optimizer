...
describe('tier3: create_credential', () => {
  it('executes RPC and returns credential resource ID', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const result = await callRpcService('create_credential', { p_name: `cred-${suffix}`, p_config: {}, p_project_id: projectId, p_doppler_project: `doppler-${suffix}` })
