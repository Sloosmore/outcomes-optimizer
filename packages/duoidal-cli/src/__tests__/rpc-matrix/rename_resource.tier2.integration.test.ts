...
describe('tier2: rename_resource', () => {
  it('renames a resource via resource rename and returns success', async () => {
    await withTestUser(async ({ jwt, assert }) => {
      ...
      const skillResult = await callRpcService('create_skill', { p_name: `skill-to-rename-${suffix}`, p_config: { content: '# Test' }, p_project_id: projectId })
      ...
      const result = await callRpc('rename_resource', {
        ...
        p_new_name: `skill-renamed-${suffix}`,
      }, jwt)
