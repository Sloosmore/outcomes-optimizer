...
describe('tier2: update_skill_content', () => {
  it('updates a skill content via resource update and returns success', async () => {
    await withTestUser(async ({ jwt, assert }) => {
      ...
      const result = await callRpc('update_skill_content', {
        ...
        p_content: '# Updated Skill\n\nThis is updated content for the tier2 integration test. It must be longer than one hundred characters.',
      }, jwt)
