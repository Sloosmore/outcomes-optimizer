...
describe('tier1: init_process', () => {
  it('initializes a process via agent-core CLI and returns process ID', async () => {
    await withTestUser(async ({ assert }) => {
      const suffix = Math.random().toString(36).slice(2, 10)

      // init_process is service-role only; agent-core process init uses the same RPC
