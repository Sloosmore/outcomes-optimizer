...
describe('tier3: create_agent', () => {
  it('executes RPC and returns agent resource ID', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const result = await callRpcService('create_agent', { p_name: `agent-${suffix}`, p_config: { content: '# Test Agent' }, p_project_id: projectId })
