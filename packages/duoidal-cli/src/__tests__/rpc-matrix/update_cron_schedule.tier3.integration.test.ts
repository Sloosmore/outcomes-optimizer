...
describe('tier3: update_cron_schedule', () => {
  it('executes RPC and returns resource ID', async () => {
    await withTestUser(async ({ jwt, assert }) => {
      ...
      const resourceId = (Array.isArray(cronResult.data) ? cronResult.data[0] : cronResult.data as Record<string, unknown>)?.['cron_resource_id'] as string | undefined
      assert(typeof resourceId === 'string', `Expected cron ID`)

      const result = await callRpc('update_cron_schedule', {
        p_resource_id: resourceId!,
        p_schedule: '30 * * * *',
        p_prompt: 'updated prompt',
        p_enabled: false,
      }, jwt)
