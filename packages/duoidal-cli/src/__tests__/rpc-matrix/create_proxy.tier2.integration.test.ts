...
describe('tier2: create_proxy', () => {
  it('creates a proxy resource and returns proxy resource ID', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const result = await callRpcService('create_proxy', {
        p_name: `proxy-${suffix}`,
        p_config: { url: 'https://proxy.test.example' },
