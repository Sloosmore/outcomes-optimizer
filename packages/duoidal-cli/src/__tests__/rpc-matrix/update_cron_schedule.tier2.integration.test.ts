...
describe('tier2: update_cron_schedule', () => {
  it('updates a cron schedule via generic update and returns success', async () => {
    await withTestUser(async ({ jwt, assert }) => {
      ...
      const result = await callRpc('update_cron_schedule', {
        p_resource_id: cronId!,
        p_schedule: '30 * * * *',
        p_prompt: 'updated prompt for tier2',
        p_enabled: false,
      }, jwt)
