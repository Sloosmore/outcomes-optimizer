...
describe('tier2: create_credential', () => {
  it('creates a credential resource and returns credential resource ID', async () => {
    await withTestUser(async ({ assert }) => {
      ...
      const result = await callRpcService('create_credential', {
        p_name: `cred-${suffix}`,
        p_config: {},
        p_project_id: projectId,
        p_doppler_project: `doppler-${suffix}`,
      })
      assert(result.ok, `RPC should succeed, got status ${result.status}: ${JSON.stringify(result.data)}`)

      const rows = Array.isArray(result.data) ? result.data : [result.data]
      const credId = (rows[0] as Record<string, unknown>)?.['credential_resource_id'] as string | undefined
      assert(typeof credId === 'string' && credId.length > 0, `Expected credential_resource_id, got: ${JSON.stringify(result.data)}`)
