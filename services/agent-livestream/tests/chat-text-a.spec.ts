...
const TURNS = [
  'I want to build an automated Instagram post scheduler',
  'It should queue posts and publish at optimal times',
  'The constraint is it must use the existing agent-instagram skill',
  'Can you show me the architecture as a diagram?',
...
test.describe('Campaign A — Instagram Scheduler', () => {
  test('5-turn conversation: 10 DB rows, tool_calls, artifact, goal reference', async ({ page }) => {
    // Turn 1: navigate to /chat/new with autostart + message
    ...
    // Wait for turn 1 to complete (orb returns to idle after being non-idle first)
    ...
    for (let i = 1; i < TURNS.length; i++) {
      ...
      // After turn 4 (i=3): check artifact-mermaid is visible
      if (i === 3) {
        await expect(page.locator('[data-testid="artifact-mermaid"]').first()).toBeVisible({ timeout: 10_000 })
      }
    }

    // Assert: 10 DB rows after 5 turns
    const allMessages = await pollMessages(page, chatId, 10)
    expect(allMessages).toHaveLength(10)

    // Turn 3 assistant message (index 5 = 6th row, 0-based) has tool_calls with claude_code
    ...
    // Turn 3 response includes AGENT_INSTAGRAM_STATE_DIR
    expect(turn3Assistant.content).toContain('AGENT_INSTAGRAM_STATE_DIR')

    // Turn 5 response references goal
    const turn5Assistant = at(allMessages, 9)
    expect(turn5Assistant.role).toBe('assistant')
    expect(turn5Assistant.content.toLowerCase()).toMatch(/goal/)
  })
})
