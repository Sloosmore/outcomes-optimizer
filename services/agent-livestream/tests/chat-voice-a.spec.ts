...
function loadFixture(turn: number): string {
  const filepath = join(FIXTURES_DIR, `voice-a-turn-${turn}.pcm16`)
...
test.describe('Voice Campaign A — YouTube Analytics', () => {
  ...
  test('5-turn voice campaign', async ({ page }) => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      // Wait for both user and assistant transcripts for this turn
    ...
    expect(lastAssistant?.content.toLowerCase()).toMatch(/youtube|analytics|dashboard/)
  })
})
