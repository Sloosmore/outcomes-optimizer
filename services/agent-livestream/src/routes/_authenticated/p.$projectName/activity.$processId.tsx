...
export const Route = createFileRoute('/_authenticated/p/$projectName/activity/$processId')({
...
function ActivityPage() {
  ...
  const { data: allProcesses = [] } = useQuery<ApiProcess[]>({
    queryKey: ['processes', 'all', projectId],
    queryFn: async () => { return apiFetch(`/api/processes?project=${encodeURIComponent(projectId)}`).then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json() }) },
  })
  ...
  const projectQs = `projectId=${encodeURIComponent(projectUuid)}`
  const graphUrl = `/api/graph${typesQs ? `?${typesQs}&${projectQs}` : `?${projectQs}`}`

  const { data, isPending } = useQuery({
    queryKey: ['graph', projectUuid, { activeTypes }],
